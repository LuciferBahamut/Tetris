...
static void put_stats_in_struct(char **split, tetris_t *t, int nbr)
{
    for (int i = 0; i != 2; i++)
...
static void get_tetriminos_shape(char *buff, tetris_t *t, int nbr)
{
    int i = 0;
    int j = 0;
...
void stock_stats(tetris_t *t, char *buff, long int size, int nbr)
{
    ...
    char **split = malloc(sizeof(char *) * 4);
    ...
    if (get_space(stat) != 2)
        t->valid[nbr] = 0;
    for (int j = 0; j != 3; j++)
        split[j] = malloc(sizeof(char) * (my_strlen(stat) + 1));
